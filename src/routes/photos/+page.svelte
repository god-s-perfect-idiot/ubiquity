...
{#if showPhotoView && selectedPhoto}
	...
	<div class="fixed inset-0 {bgClass} z-50 flex items-center justify-center page-holder">
		<div class="relative w-full h-full page overflow-hidden" class:page-exit={isExiting}>
			<div
				class="absolute top-4 left-4 right-4 {bgClass} bg-opacity-50 {textClass} px-4 py-2 rounded overflow-hidden"
			>
				<span class="block text-2xl truncate">{selectedPhoto.name}</span>
			</div>
				...
				<div class="photos-grid">
					{#each photos as photo, index}
						<div
							...
							style="animation-delay: {Math.min(index * 30, 360)}ms"
							on:click={() => openPhotoView(photo)}
						>
