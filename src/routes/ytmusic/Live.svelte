<script>
	...
	// Only show YTMusic tracks
	$: ytTrackData = currentTrackData && serviceType === 'ytmusic' ? currentTrackData : null;

	// Make these reactive so they update when currentTrackData changes
	$: trackDisplayName = ytTrackData ? ytTrackData.name || 'Unknown track' : 'No track playing';
	$: trackArtist = ytTrackData?.artists
		? Array.isArray(ytTrackData.artists)
			? ytTrackData.artists.map((a) => a.name).join(', ')
			: 'Unknown Artist'
		: 'Unknown Artist';
	$: backgroundStyle =
		ytTrackData?.album?.images && ytTrackData.album.images.length > 0
			? `background-image: url('${ytTrackData.album.images[0].url}'); background-size: cover; background-position: center;`
...
{#if gridSize === '2x2'}
	<!-- 2x2 Tile: Compact YTMusic player -->
	<div
	...
	>
		...
		{:else}
			...
			<div class="flex flex-col items-center justify-center h-full gap-2">
				<Icon icon="simple-icons:youtubemusic" width="56" height="56" />
				<span class="text-base font-medium left-2 bottom-2 absolute">YTMusic</span>
			</div>
		{/if}
	</div>
{:else if gridSize === '4x2'}
	<!-- 4x2 Tile: Full YTMusic player with album art background -->
	<div
	...
	>
		...
		{#if ytTrackData}
			...
			<div class="flex flex-col justify-between h-full flex-1 min-w-0 z-10">
				<div class="flex flex-row justify-between items-center gap-3">
					...
					<Icon icon="simple-icons:youtubemusic" width="48" height="48" />
				</div>
		...
		{:else}
			...
			<div class="flex flex-col items-center justify-center h-full w-full gap-3 z-10">
				<Icon icon="simple-icons:youtubemusic" width="72" height="72" />
				<span class="absolute bottom-2 left-2 text-base font-medium">YTMusic</span>
			</div>
		{/if}
	</div>
{/if}
